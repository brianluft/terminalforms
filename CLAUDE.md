...
- We do not use tvision's command system. Instead, we override the functions that send messages and rig them to invoke C# callbacks instead.

# C++ Rules
- Update `CMakeLists.txt` to include any new source file that you create.
- Always use the `tf` namespace for C++ classes and internal implementation details. Put `TF_EXPORT` (`extern "C"`) functions in the global namespace.
...
- Always return `tf::Error` for error handling. Never allow exceptions to bubble up past the C interface. Feel free to add new error codes to `src\tfcore\common.h` (`Error` enum) and make the corresponding change in `src\TerminalForms\Error.cs`--these two files must be kept synchronized.
...
TF_EXPORT tf::Error TfFooNew2(Foo** out, int32_t bar, TWhatever* baz) {
...
// ... TF_EXPORT function implementations ...
```

- See `src/tfcore/common.h` for various TF_* macros you can use in your implementation. Don't provide function prototypes for the exported C methods in the .h file, as they will be consumed from C# which doesn't use the header files and just wastes our time.
...
- Always write XML doc comments on public members. This will be seen by users and it's their primary way to learn about our library. Be thoughtful about how application developers will use ths functionality, and go deep, researching in the books as necessary for inspiration. However, do not describe invisible implementation details. Do not mention C++ or Turbo Vision. This is a C# library and the internal details will confuse the C# programmer.
- When writing demo tests, you cannot write the expected output files as they are terminal screenshots; you must `NO_TESTS=1 scripts/build.sh` then `scripts/run-demo.sh --test FooDemo --output <txt file path>` and it will write the file. You can use `grep` to see if expected strings are present.
...
- `context\tvision-pascal-book\*.md` is the Turbo Vision for Pascal user's guide. The details don't exactly match our C++ library, but the high level types and concepts are the same, and this book is more comprehensive than the C++ book. CAUTION: Always provide answers in terms of the C++ library. The Pascal book is only for high-level guidance. We do not use Pascal in this project.
