    ...
    : NativeObject<WriteArgs>(ptr, owned, placement)
{
    ...
    {
        public Factory()
            : base(NativeMethods.TV_write_args_placementSize) { }

        public unsafe void* PlacementNew(byte* ptr)
        {
            ptr = Align(ptr);
            TurboVisionException.Check(NativeMethods.TV_write_args_placementNew(ptr));
            return ptr;
        }

        public static unsafe void* New()
        {
            TurboVisionException.Check(NativeMethods.TV_write_args_new(out var ptr));
            return ptr;
        }
    }
