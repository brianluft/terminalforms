...
/// <summary>
/// Represents a collection of string items in a <see cref="RadioButtonGroup"/> control.
/// This collection maintains synchronization between the managed list and the native
/// Turbo Vision string collection.
/// </summary>
/// <remarks>
/// The collection supports standard list operations such as adding, removing, and modifying items.
/// Changes to the collection are immediately reflected in the visual display of the radio button group.
/// When items are removed, the selected index is automatically adjusted to maintain a valid selection.
/// </remarks>
public unsafe partial class RadioButtonItemCollection : IList<string>
{
    private readonly RadioButtonGroup _owner;
    private readonly List<string> _items = [];

    internal RadioButtonItemCollection(RadioButtonGroup owner)
    {
        _owner = owner;
    }
    ...
    public string this[int index]
    {
        ...
        set
        {
            ...
            if (index < 0 || index >= _items.Count)
                throw new ArgumentOutOfRangeException(nameof(index));
            _items[index] = value;
            Check(NativeMethods.TfRadioButtonGroupSetItemAt(_owner.Ptr, index, value));
        }
    }
    ...
    public void Add(string item)
    {
        ...
        _items.Add(item);
        Check(NativeMethods.TfRadioButtonGroupAddItem(_owner.Ptr, item));
    }
    ...
    /// </summary>
    /// <remarks>
    /// After clearing the collection, the radio button group will have no items to display.
    /// The selected index will be reset.
    /// </remarks>
    public void Clear()
    {
        ObjectDisposedException.ThrowIf(_owner.IsDisposed, _owner);
        _items.Clear();
        Check(NativeMethods.TfRadioButtonGroupClearItems(_owner.Ptr));
    }
    ...
    public void Insert(int index, string item)
    {
        ...
        Check(NativeMethods.TfRadioButtonGroupInsertItemAt(_owner.Ptr, index, item));
    }
    ...
    /// <list type="bullet">
    /// <item>If the removed item was selected, the previous item (or first item) becomes selected.</item>
    ...
    public void RemoveAt(int index)
    {
        ...
        Check(NativeMethods.TfRadioButtonGroupRemoveItemAt(_owner.Ptr, index));
    }
    ...
    /// Called during RadioButtonGroup construction.
    /// </summary>
    internal void SyncFromNative()
    {
        _items.Clear();
        Check(NativeMethods.TfRadioButtonGroupGetItemCount(_owner.Ptr, out var count));
        for (var i = 0; i < count; i++)
        {
            Check(NativeMethods.TfRadioButtonGroupGetItemAt(_owner.Ptr, i, out var item));
            _items.Add(item);
        }
    }
    ...
    private static partial class NativeMethods
    {
        ...
        public static partial Error TfRadioButtonGroupGetItemCount(void* self, out int @out);

        [LibraryImport(Global.DLL_NAME, StringMarshalling = StringMarshalling.Utf8)]
        public static partial Error TfRadioButtonGroupGetItemAt(
            void* self,
            int index,
            out string @out
        );
        ...
        [LibraryImport(Global.DLL_NAME, StringMarshalling = StringMarshalling.Utf8)]
        public static partial Error TfRadioButtonGroupSetItemAt(void* self, int index, string text);

        [LibraryImport(Global.DLL_NAME, StringMarshalling = StringMarshalling.Utf8)]
        public static partial Error TfRadioButtonGroupAddItem(void* self, string text);

        [LibraryImport(Global.DLL_NAME, StringMarshalling = StringMarshalling.Utf8)]
        public static partial Error TfRadioButtonGroupInsertItemAt(
            void* self,
            int index,
            string text
        );
        ...
        public static partial Error TfRadioButtonGroupRemoveItemAt(void* self, int index);

        [LibraryImport(Global.DLL_NAME)]
        public static partial Error TfRadioButtonGroupClearItems(void* self);
    }
}
