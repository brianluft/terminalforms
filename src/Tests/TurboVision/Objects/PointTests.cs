...
public sealed class PointTests
{
    ...
    {
        var bytes = stackalloc byte[TPoint.PlacementSize];
        using var point = new TPoint(bytes);
    ...
    {
        using var a = new TPoint { X = 1, Y = 2 };
        using var b = new TPoint { X = 1, Y = 2 };
        using var c = new TPoint { X = 3, Y = 4 };
    ...
    {
        using var a = new TPoint { X = 1, Y = 2 };
        using var b = new TPoint { X = 3, Y = 4 };
        a.Add(b);
    ...
    {
        using var a = new TPoint { X = 3, Y = 4 };
        using var b = new TPoint { X = 2, Y = 1 };
        a.Subtract(b);
    ...
    public void Test_Subtract_Static()
    {
        ...
        using var b = new TPoint { X = 2, Y = 1 };
        using var c = new TPoint();
        TPoint.Subtract(a, b, c);
    }

    [TestMethod]
    public void Test_X()
    {
        using var point = new TPoint { X = 1, Y = 2 };
        Assert.AreEqual(1, point.X);
        point.X = 3;
